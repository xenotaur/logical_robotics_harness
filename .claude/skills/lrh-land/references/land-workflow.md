...
| **Main-worktree-lock** | When all worktrees have `main` checked out: `git fetch → checkout -b tmp-<slug> origin/main → apply changes → push tmp-<slug>:main → checkout <pr-branch> (or --detach) → delete tmp-<slug>` — the checkout-away step is required because Git refuses to delete the branch `HEAD` currently points to |
