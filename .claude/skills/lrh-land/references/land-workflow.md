...
| **Main-worktree-lock** | When all worktrees have `main` checked out: `git fetch → checkout --detach origin/main → apply changes and commit (stay detached until the closeout commit(s) exist) → push origin HEAD:main → checkout <pr-branch>` — no local branch is created, so there is nothing to delete (the project's `permissions.deny` blocks `git branch -D`/`-d`). Never push or check out `<pr-branch>` before the closeout commit(s) are made, or they land on the wrong branch and the push omits them; check out away only after the push succeeds: until then the commits are reachable only from the detached `HEAD` and the reflog — the explicit `origin` is required (a bare `HEAD:main` argument is parsed as a repository, not a refspec). **Capture the branch point when detaching, to a file, not a shell variable** — most agent harnesses run each Bash invocation as a separate process with no persisted shell state (only the working directory carries over), so a plain `tmp_branch_parent=$(...)` set at detach time will not survive to a later tool call that needs it. Write it where a later call can still read it — resolve the real git directory first, never hardcode `.git/`: in a worktree checkout, `.git` is a *file* containing a gitdir pointer, not a directory, so a literal `.git/lrh-tmp-branch-parent-<slug>` path fails (`Not a directory`). Use `$(git rev-parse --git-dir)`, which correctly resolves to the real git directory whether `.git` is a plain directory or a worktree pointer file: `git rev-parse origin/main > "$(git rev-parse --git-dir)/lrh-tmp-branch-parent-<slug>"` right alongside the `checkout --detach`, and read it back with `tmp_branch_parent=$(cat "$(git rev-parse --git-dir)/lrh-tmp-branch-parent-<slug>")` wherever the troubleshooting rows below need it. Remove the file (`rm "$(git rev-parse --git-dir)/lrh-tmp-branch-parent-<slug>"`) once the push succeeds and the checkout-away step runs — it has no further use once this run's closeout is done |
...
| Symptom | Resolution |
|---|---|
| **Non-fast-forward on push** | If `git push origin HEAD:main` is rejected as non-fast-forward, this usually means an unrelated commit landed on `main` concurrently — not a conflict with this run's own changes. Check as two separate commands, not one chained with `&&` (see the next row — this check is itself exactly the compound-command shape that can be denied ambiguously): first `git fetch origin main`, then `git merge-base --is-ancestor "$tmp_branch_parent" origin/main` (using the value captured at branch creation, per the Main-worktree-lock row above — never re-derive it from the current `origin/main`, which has already moved). If it reports an ancestor, this is the clean-rebase case — run `git rebase origin/main` (on the detached `HEAD`). Ancestry alone does not guarantee a conflict-free rebase: if the rebase completes cleanly, retry the push; if it stops on a conflict, do not force through it or retry blindly — `git rebase --abort`, then stop and report the conflicting files for a human decision. If the ancestry check itself reports false (`$tmp_branch_parent` is not an ancestor of `origin/main`), this is not the benign concurrent-commit case — stop and report rather than rebasing against unrelated history. Never treat any of these outcomes as a stall or a policy block |
| **Ambiguous permission denial** | If a chained/compound Bash call (multiple git operations joined by `&&` in one tool call) is denied by the permission layer with wording that reads like a policy or gate block, **first check whether any individual command in the chain matches this project's own `permissions.deny` list** (`docs/how-to/project-setup/claude-code-permissions.md` § What is deliberately still denied — e.g. `git push --force`, `git reset --hard`, `git checkout .`/`git restore`, `git branch -D`/`-d`, `git clean`, `rm -rf`). `permissions.deny` is an unconditional block that takes precedence over any allow rule — if a command in the chain genuinely matches, that is a real, intentional denial, not classifier ambiguity: stop and report it, do not retry in any form, chained or split. The Main-worktree-lock flow uses a detached `HEAD` precisely so it never needs `git branch -D`/`-d`; if that flow appears to require one, stop and report. Otherwise, when no command in the chain matches a deny rule, the ambiguity applies as normal — retry the same action as a single, minimal command first, since compound commands can be classified ambiguously independent of whether each individual command is itself safe |
