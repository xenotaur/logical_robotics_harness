...
description: 'Inspect and change the chain-defaults gate policy in project/config/chain-defaults.yaml.
  ...
  '
when_to_use: 'Invoke when the user wants to inspect or change chain-authorization
  gate behavior for /lrh-land or /lrh-execute -- e.g. "what''s our current gate config",
  "flip confirm_fixes_batch", "grant consent for skip_if_opted_in". Do not use to
  change any other project config, and do not use it to make a gate decision on the
  user''s behalf -- every change is presented and confirmed, never inferred.
