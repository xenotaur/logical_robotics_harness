...
when_to_use: 'Invoke when the user wants to inspect or change this repository''s lrh
  skills install defaults -- e.g. "what''s our current skills-install config", "set
  the skill source to current-repo for this repo", "make codex the default target
  here". Do not use to change any other project config, and do not use it to make
  install.overwrite editable -- that field is display-only in this skill.
