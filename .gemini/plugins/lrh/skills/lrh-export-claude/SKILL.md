...
description: 'Export the current or specified Claude Code session transcript into
  a private, non-authoritative Markdown export artifact. Use when the user asks to
  export, capture, or archive a Claude Code conversation session. Wraps `lrh conversation
  export-claude-session`, verifies the artifact with `lrh conversation inspect-export`,
...
when_to_use: "Invoke only when the user explicitly asks to export, capture, or archive\
  \ a Claude Code session transcript for later private review. Do not invoke proactively\
  \ or infer the intent from a passing mention of exporting \u2014 by default this\
  \ skill writes a durable, permanent copy into the user's local session archive (not\
  \ a self-cleaning temp file), so invocation should always trace to a direct request,\
  \ not an agent's own judgment call. With no discovery flag, defaults to exporting\
  \ the current session. A user-typed `/lrh-export-claude` invocation is itself the\
