---
name: lrh-implement
description: 'Implement an LRH work item or ad-hoc task using the three-phase execution
  session model. Given a work item ID or task description, this skill mints a prompt
  ID, checks idempotence, confirms the implementation plan, creates a branch, implements
  the changes, runs canonical validation, opens a PR, and creates a populated execution
  record. Use when the user wants to execute a defined work item (WI-*) or an ad-hoc
