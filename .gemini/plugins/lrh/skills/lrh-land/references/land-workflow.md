...
| **Ambiguous permission denial** | If a chained/compound Bash call (multiple git operations joined by `&&` in one tool call) is denied by the permission layer with wording that reads like a policy or gate block, **first check whether any individual command in the chain matches this project's own `permissions.deny` list** (`docs/how-to/project-setup/claude-code-permissions.md` § What is deliberately still denied — e.g. `git push --force`, `git reset --hard`, `git checkout .`/`git restore`, `git branch -D`/`-d`, `git clean`, `rm -rf`). `permissions.deny` is an unconditional block that takes precedence over any allow rule — if a command in the chain genuinely matches, that is a real, intentional denial, not classifier ambiguity: stop and report it, do not retry in any form, chained or split. Only when no command in the chain matches a deny rule does the ambiguity apply — in that case, retry the same action as a single, minimal command first, since compound commands can be classified ambiguously independent of whether each individual command is itself safe |
