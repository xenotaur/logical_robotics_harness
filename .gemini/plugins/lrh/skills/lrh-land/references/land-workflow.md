...
| Symptom | Resolution |
|---|---|
| **Non-fast-forward on push** | If `git push origin HEAD:main` is rejected as non-fast-forward, this usually means an unrelated commit landed on `main` concurrently — not a conflict with this run's own changes. Check as two separate commands, not one chained with `&&` (see the next row — this check is itself exactly the compound-command shape that can be denied ambiguously): first `git fetch origin main`, then `git merge-base --is-ancestor "$closeout_base" origin/main` (using the value captured at detach time, per the Main-worktree-lock row above — never re-derive it from the current `origin/main`, which has already moved). If it reports an ancestor, this is the clean-rebase case — run `git rebase origin/main` (on the detached `HEAD`). Ancestry alone does not guarantee a conflict-free rebase: if the rebase completes cleanly, retry the push; if it stops on a conflict, do not force through it or retry blindly — `git rebase --abort`, then stop and report the conflicting files for a human decision. If the ancestry check itself reports false (`$closeout_base` is not an ancestor of `origin/main`), this is not the benign concurrent-commit case — stop and report rather than rebasing against unrelated history. Never treat any of these outcomes as a stall or a policy block |
