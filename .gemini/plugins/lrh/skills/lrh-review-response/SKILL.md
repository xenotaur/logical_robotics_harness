...
description: 'Address open review comments on an LRH pull request. Runs lrh request
  review_response to fetch open comments, shows them for confirmation, mints a prompt
  ID for traceability, triages and addresses each comment following the embedded protocol,
  validates, and pushes fixes to the open PR. Invoke in the same session that created
  the PR for best design-context continuity. Provide the PR URL as the argument, or
  omit to auto-detect from the current branch.

  '
when_to_use: 'Invoke when the user wants open PR review comments addressed and fixed,
  or as a link in a human-initiated /lrh-land chain. Do not invoke merely because
  a PR is mentioned with no request to address its comments. The Step 4 confirm gate
