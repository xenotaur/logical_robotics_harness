...
description: "Dispatch a fresh, cold-context subagent to independently review a diff\
  \ or a PR \u2014 a credit-free substitute for a GitHub bot review. Two modes: diff-mode\
...
when_to_use: 'Invoke only at the two declared review-substitution trigger points:
  once in diff-mode from /lrh-implement before a PR is opened, or in PR-mode from
  /lrh-confirm-fixes when a fresh substitute review signal is needed instead of a
  hosted GitHub review-bot retrigger. Do not use for ad-hoc review outside those caller-owned
  workflows.
