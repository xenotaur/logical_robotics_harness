...
- `/lrh-export-claude` — Export the current or specified Claude Code session transcript into a private, non-authoritative Markdown export artifact; a user-typed invocation proceeds on its own, a model-initiated one still requires confirmation before writing, and terminal reporting is metadata-only
