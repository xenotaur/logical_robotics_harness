...
- `setup_state` semantics are now truth-first: local-path locators are checked as `lrh_project_present` vs `not_set_up`, while remote URL locators remain `not_checked` unless explicit remote probing is implemented
