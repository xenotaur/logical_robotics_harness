...
- for GitHub tree URLs (for example `.../tree/main/project`), `meta register` now derives default registry/short/display names from the repository slug, normalizes stored `repo_locator` to `.../tree/<ref>`, and stores `project_dir` as the tail path identity
