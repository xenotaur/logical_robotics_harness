...
- [CLI reference](cli/README.md) — command-oriented reference material, including docs-request workflows.
- [Schema reference](schemas/README.md) — project-control and file-format reference material.
- [Agent skills config](schemas/agent-skills-config.md) — optional `project/agent_skills.yaml` schema for repository-local skill install defaults.
- [Desktop server protocol](desktop-server-protocol.md) — versioned machine contract for a desktop supervisor that owns an `lrh serve --desktop-protocol` child.
