...
- `FRONTMATTER_LINT_UNSAFE_SCALAR` is a report-only warning category that flags unquoted frontmatter plain scalars that either crash real YAML parsing or silently change meaning under it (an unquoted colon-collapse, an unquoted mid-scalar `#` comment, a reserved-indicator-leading scalar, or a known string field that would implicitly resolve to a non-string type). It never fails validation on its own and never rewrites anything — see [`lrh project doctor --fix-frontmatter`](doctor.md) for the corresponding one-time content-migration tool.
