...
| `invalid_workspace` | Path relative, missing, not a directory, unusable (for example a name too long or no permission), or over 4,096 bytes (as requested or once resolved). | Ask the user to fix Settings. |
...
| `internal_error` | Unexpected backend exception before `ready` (details on stderr); carries the launch ID once a `start` has been read. | Show failed with stderr tail. |
