...
clone.** Each such clone is a genuinely separate Claude Code bucket
(`project_slug_for_path`, `src/lrh/prompt_workflow_sessions.py:565-582`,
keys purely on the literal working-directory path string). **This is a
narrow scope boundary of this one document, not a claim that those
clones are intended to start without memory access.** They should have
it — that they currently don't is a real, known gap, not a design
choice: `git worktree`'s own documented purpose is isolating working
directories/branches to avoid contention (see
`git-scm.com/docs/git-worktree`, "a git repository can support multiple
working trees, allowing you to check out more than one branch at a
time" — nothing about isolating information or memory access), so
nothing about wanting per-clone git isolation implies wanting per-clone
memory isolation. Closing that gap — syncing relevant memory into (and
between) `Workspaces/*` clones — is being designed separately as a
hub-and-spoke system; do not treat a `Workspaces/*` clone's absence from
this table's migration columns as evidence the gap has been decided
against.
