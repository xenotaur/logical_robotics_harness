...
LRH resolves project state using a deterministic precedence chain:

```
principles > goal > roadmap > focus > work_items > guardrails > runtime
```

The canonical definition is maintained in
`project/memory/decisions/precedence_semantics.md`.

This document intentionally summarizes, rather than re-specifies, the semantics.

## Practical summary

- Higher-authority layers constrain lower-authority layers.
- Lower-authority layers refine and narrow scope; they do not override higher-authority constraints.
- Guardrails are subtractive constraints: they can remove options but do not define positive scope.
- Runtime invocation is narrowing-only: it can select from allowed focus/work/contributors, but cannot reintroduce blocked entities.
- Memory informs planning and interpretation but is not a precedence layer.

## Why this matters

The precedence model keeps LRH deterministic and auditable by making conflict resolution explicit. In practice it governs:

- what focus is active
- which work items are in scope
- which contributors are eligible in the current run

For exact invariants and change-control requirements, refer to the canonical decision record.
