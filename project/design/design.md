...
LRH records decisions in two tiers:

- **`project/memory/decision_log.md`** — the default landing spot. A
  chronological, append-only log of dated entries; this is where every
  decision starts.
- **`project/memory/decisions/<slug>.md`** — a promoted, single-topic file
  for a decision that other documents need to cite independently and
  repeatedly, outside the log's chronological narrative. Promote a log entry
  into its own file when it becomes a load-bearing invariant referenced from
  multiple other documents — the way
  `project/memory/decisions/precedence_semantics.md` was promoted out of the
  log because `architecture.md`, `repository_spec.md`, `AGENTS.md`, and
  several work items all needed to cite it directly.
