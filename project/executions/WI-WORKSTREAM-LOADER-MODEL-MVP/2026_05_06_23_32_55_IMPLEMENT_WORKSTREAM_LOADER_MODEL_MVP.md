...
status: in_progress
rerun_of: 
pr: xenotaur/logical_robotics_harness#197
commit: 
...
`ProjectState`, and exported `load_workstreams` from `lrh.control`. Added focused unittest
coverage for minimal proposed loading, multiple bucket loading, ignored
README/placeholder/non-workstream Markdown files, source metadata preservation, predictable optional
defaults, and list-field loading. Updated `project/workstreams/README.md` to describe the current
loader/model status and note that directory-style workstreams remain future work.
...
- After merge, set this execution record status to `landed` and fill final commit metadata.
