...
This decision defines how LRH models and integrates Google Antigravity as an agent execution environment alongside Claude Code and the Codex App. Antigravity discovers and executes skills via two distinct mechanisms: (1) native plugin manifests (`~/.gemini/config/plugins/<plugin>/plugin.json`) for ambient system-prompt indexing, and (2) on-demand direct file inspection of canonical in-repo `SKILL.md` files (in `src/lrh/skills/` or `.claude/skills/`) guided by project-level (`AGENTS.md`) or user-level (`~/.gemini/GEMINI.md`) rules.
...
- Antigravity uses a plugin/skill architecture where skills center around `SKILL.md` (YAML frontmatter + Markdown body prose) housed under `~/.gemini/config/plugins/<plugin>/skills/<skill>/SKILL.md`.
- Antigravity also parses workspace `AGENTS.md` and user-level `~/.gemini/GEMINI.md` files into `<user_rules>` at session initialization, enabling zero-install direct reading of in-repo canonical skill instructions via file-viewing tools.
