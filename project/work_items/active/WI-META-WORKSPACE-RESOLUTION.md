...
- 2026-04-23: Implemented `lrh meta inspect` as a truth-first single-record inspection path that reuses shared workspace resolution, renders stored registry fields explicitly, and includes minimal derived path/existence checks for local-path locators.
- 2026-04-23: Implemented `lrh meta where` as a read-only workspace inspectability command (human + `--json` output) wired to the shared resolver, with clear resolution-failure guidance and focused CLI tests.
