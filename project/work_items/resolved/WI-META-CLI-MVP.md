...
- Follow-on work item: `project/work_items/resolved/WI-META-WORKSPACE-RESOLUTION.md` (shared workspace-context resolution and visibility behavior)
...
- 2026-04-23: Added focused compatibility validation for `lrh meta register`/`lrh meta list` under hybrid/local/global modes, including resolved `projects_dir` write/read behavior, normalized-config path handling, and malformed path-value fallback/error coverage.
- 2026-04-23: Added non-interactive passthrough support to `scripts/sandbox` (while preserving interactive behavior) plus smoke coverage, so maintainers/agents can safely run `meta init`/`meta where` against isolated HOME/XDG/TMP paths without touching real user runtime state.
