...
The repository-local scripts are compatibility wrappers that delegate to `lrh prompt ...` (or fall back to `python -m lrh.cli.main prompt ...` in a local checkout). Use installed `lrh prompt ...` commands as the portable interface for client repositories.
