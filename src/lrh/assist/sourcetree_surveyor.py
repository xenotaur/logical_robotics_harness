...
def _should_skip_path(path: pathlib.Path) -> bool:
    return any(_should_skip_dir(part) for part in path.parts)


def _walk_files(root: pathlib.Path, pattern: str) -> typing.Iterator[pathlib.Path]:
    """Yields files matching pattern using os.walk for fast directory pruning."""
    import fnmatch
    import os
...
def scan_tree(
    ...
    for path in sorted(_walk_files(root, "*.py")):
...
def survey_python_tree(
    ...
    if effective_tests_root is not None:
        for test_path in sorted(_walk_files(effective_tests_root, "*.py")):
