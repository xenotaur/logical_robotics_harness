...
def render_codex_cloud_prompt(data: WorkItemPromptData) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"- Prompt ID: `{data.prompt_id}`",
            f"- If `AGENTS.md` exists, read and follow it.",
            f"- If `{data.style_guide_path}` exists, read and follow it.",
            "- If `PROMPTS.md` exists, follow its prompt-ID and execution-record conventions.",
            "- If guidance files are missing, use local conventions and report bootstrapping gaps.",
            ...
            "- If unavailable but `PROMPTS.md` and `project/executions/` exist, record execution manually.",
            "- If prompt workflow scaffolding is absent, report execution recording was skipped.",
            "",
            "# OUTPUT REQUIREMENTS",
            "",
