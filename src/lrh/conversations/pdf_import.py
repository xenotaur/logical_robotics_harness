...
def _pdf_text_sources(raw_bytes: bytes) -> list[str]:
    stream_sources = _decode_pdf_streams(raw_bytes)
    if stream_sources:
        return stream_sources
    return [raw_bytes.decode("latin-1", errors="ignore")]


def _decode_pdf_streams(raw_bytes: bytes) -> list[str]:
    sources: list[str] = []
    for match in re.finditer(
        rb"<<(.*?)>>\s*stream(?:\r\n|\n|\r)?(.*?)(?:\r\n|\n|\r)?endstream",
        raw_bytes,
        ...
        stream_dictionary = match.group(1)
        stream_data = match.group(2)
        if _stream_uses_flate_decode(stream_dictionary):
            try:
                stream_data = zlib.decompress(stream_data)
            except zlib.error as exc:
                raise PdfImportError("Unable to decode FlateDecode PDF stream") from exc
        sources.append(stream_data.decode("latin-1", errors="ignore"))
    return sources


def _stream_uses_flate_decode(stream_dictionary: bytes) -> bool:
    return (
        re.search(rb"/Filter\s*(?:\[[^\]]*)?/FlateDecode\b", stream_dictionary)
        is not None
    )
...
def _extract_pdf_text(
    ...
    raw_bytes = pdf_path.read_bytes()
    chunks: list[str] = []
    for source in _pdf_text_sources(raw_bytes):
        chunks.extend(_extract_text_showing_chunks(source))
    if not chunks:
