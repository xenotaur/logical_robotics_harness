...
All names here are public - this module has no internal/external
distinction to make, since being importable by other subsystems is its
entire purpose (PR #604 review, `copilot-pull-request-reviewer`).
"""
...
CREDIT_CARD_CANDIDATE_PATTERN = re.compile(r"(?<!\d)(?:\d[ -]?){12,18}\d(?!\d)")
...
BASIC_RULES = (
    ...
    Rule(
        ...
        severity=SEVERITY_HIGH,
        confidence=CONFIDENCE_HIGH,
        pattern=URL_CREDENTIALS_PATTERN,
...
def is_valid_ipv4_address(candidate: str) -> bool:
