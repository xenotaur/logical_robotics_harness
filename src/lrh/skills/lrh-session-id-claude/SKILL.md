...
description: >
  Report the current or specified Claude.app session identity as an LRH
  `session_transcript: claude-app:<host-uuid-stem>` pointer, plus the title,
  branch, PR, and child-id-alias eligibility that `lrh prompt
  record-session-alias` needs, without exporting or reading transcript
  content. Use when a closeout or execution record needs the Claude session
  pointer, or when /lrh-closeout, /lrh-land, or /lrh-implement resolves one.
argument-hint: "[HOST_SESSION_ID | PR_NUMBER | PR_URL | BRANCH]"
---
