...
uppercasing (e.g. `wi-skills-lrh-setup` → `WI_SKILLS_LRH_SETUP`), then
match the complete trailing filename segment — not a bare substring, which
would also match an unrelated longer slug that happens to contain this one
(e.g. `..._WI_SKILLS_LRH_SETUP_REVIEW.md`):
