...
class TestSourcetreeSurveyorImport(unittest.TestCase):
    def test_module_imports_from_assist_package(self) -> None:
        self.assertTrue(hasattr(sourcetree_surveyor, "scan_tree"))
        self.assertTrue(callable(sourcetree_surveyor.scan_tree))
...
class TestSourcetreeSurveyorAnalysis(unittest.TestCase):
    def test_scan_tree_collects_public_and_private_symbols(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            reports = sourcetree_surveyor.scan_tree(root, tests_root)
    ...
    def test_json_and_markdown_render_expected_sections(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            reports = sourcetree_surveyor.scan_tree(root, tests_root=None)

            markdown = sourcetree_surveyor.to_markdown(reports, root, tests_root=None)
            self.assertIn("# Surface inventory:", markdown)
            self.assertIn("### Public functions", markdown)
            self.assertIn("`visible`", markdown)

            json_text = sourcetree_surveyor.to_json(reports)
            payload = json.loads(json_text)
            self.assertEqual(payload[0]["module"], "simple")
            self.assertEqual(payload[0]["functions"][0]["name"], "visible")
